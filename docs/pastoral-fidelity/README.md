...
**Última actualización**: 2026-05-25 (recierre tras deuda) — Phase 0 **cerrada con deuda 0**. PRs: #252 (8 PRs originales) + #253 (ADR-010) + #254 (smoke fixes) + #255 (Phase 0 deuda follow-up: seed CORE ingest + backfill heurístico + UI badges + useUserProfile refetch). Smoke 1-8 + Deuda 1-4 validated end-to-end via MCP — Firestore docs verificados + `confessionChangeAudit/` populated + functions `ingestLibrarySeedSources` + `changePlanForUser` deployed. Nueva **Fase 2.5 Study Depth Copilot** insertada entre Fase 2 y 3 — engine multidimensional + confrontación soft para path Faculty (cierra gap PR #214). Fase 1 (six-step spine) lista para arranque. Propuestas tracked: [faculty-sermon-rag-enrichment.md](./proposals/faculty-sermon-rag-enrichment.md) (techo citacional Faculty) + [pdf-export-rewrite.md](./proposals/pdf-export-rewrite.md) (Puppeteer migration).
