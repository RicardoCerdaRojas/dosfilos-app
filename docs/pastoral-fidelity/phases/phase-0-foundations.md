...
- [ ] Las 14 fuentes CORE del JSON ingestadas en Firestore (chunked por sección)
- [ ] Cada sección tiene `doctrineLevel` taggeado (core/distinctive/open-evangelical)
- [ ] Source schema extendido con campos rights-aware (license, ingestionStatus, etc.)
- [ ] Citation render function devuelve estilo correcto por context (sermon vs essay vs rag)
- [ ] SBLGNT render incluye atribución CC BY 4.0 obligatoria
- [ ] Usuario nuevo puede declarar confesión en onboarding (obligatorio)
- [ ] Usuario existente puede declarar via banner (no bloqueante)
- [ ] `declaredConfession` + `confessionVisibility` persisten correctamente
- [ ] `/settings/confession` permite editar (con audit log)
- [ ] Feature flag `pastoral_fidelity_flow` toggleable por admin
- [ ] TSK cross-reference dataset accesible via endpoint (lookup `(book, chapter, verse) → parallels[]`)
- [ ] Auditoría UI completa documentada (input para kill-list)
- [x] ADRs 005-008 escritos (todas las 5 preguntas iniciales cerradas)
