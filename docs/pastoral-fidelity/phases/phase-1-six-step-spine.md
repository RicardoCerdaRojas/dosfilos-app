...
`planning` — **destrabada 2026-05-23, prereqs revalidados al recierre de Fase 0 (2026-05-25)**. Fase 0 cerrada con deuda 0 (PRs #252, #253, #254, #255 en main). Lista para arranque vía `/iniciar-fase 1` en conversación nueva.
...
- **2026-05-25** — **Prereqs revalidados al recierre de Fase 0**. ADR-009 + ADR-010 cambian asunción de Phase 1: NO hay `declaredConfession` obligatorio, NO hay banner, hook `usePastoralFidelityGate` retorna `confessionalWitnessesEnabled` (default `true`) en vez de `confession-required` gate. Phase 2 Testigo 3 ajustado: `core` fire siempre + `distinctive`/`open-evangelical` fire si toggle ON. CORE Library seed con 8 system sources adicionales en `library_resources` (SBLGNT + Schaff + Chicago + minor confessions) disponibles para citation manifest desde Step 6. `useUserProfile.refetch()` disponible para flows que muten profile (settings, gate-aware). Faculty sermon RAG enrichment + PDF export rewrite tracked como proposals no-bloqueantes. Phase 1 sigue destrabada, sin deuda invisible heredada.
