...
**Versión:** 1.3.1 · 15-jun-2026 · *supersede v1.0–v1.2*
**Cambio 1.3 → 1.3.1 (vocabulario):** las "puertas" de validación pasan a llamarse **exámenes** ("examínese cada uno", 1 Co 11:28; "examinadlo todo", 1 Ts 5:21), para alinear con el manifiesto de conducción formativa. El mecanismo es idéntico; solo cambia el nombre. Los "tres testigos" no cambian.
...
- *Cristalización* = un acto explícito del docente ("este es el contenido") que congela el estudio y **dispara los siete exámenes**.
...
- *Migración* = no disruptiva. Estudios viejos = `sin_auditar`; nuevos = con elementos y exámenes.
...
Consecuencia directa que resuelve la duda recurrente: **el contenido (argumentos, distinciones, fuentes de una clase de seminario) se desarrolla SIEMPRE en el estudio, nunca en el transponedor.** El transponedor no puede agregar una afirmación doctrinal que no esté en el estudio — si la necesita, se vuelve al estudio y pasa los exámenes. Eso es lo que mantiene la fidelidad heredándose en vez de reinventándose en cada destino.
...
El Estudio Madre es el *qué dice el texto / qué afirma la doctrina*, con los exámenes en verde. **No tiene forma de entregable**: no es un sermón, ni una clase, ni una guía. Es la materia prima fiel de la que cualquiera de esos sale. Es el equivalente del cuaderno del pastor antes de decidir si eso será sermón del domingo o clase del miércoles.
...
- **Extendible** — se vuelve a la conversación, se trabaja un elemento nuevo, se promueve. (Una afirmación doctrinal nueva vuelve a pasar su examen.)
...
4. Presiona **"este es el contenido de la clase"** → acto de cristalización: congela el Estudio Madre y **dispara los siete exámenes** (§8). Si pasa → `estado_fidelidad: verde`, listo para transponer. Si no → el sistema dice qué falta antes de dejar cerrar.

Por eso el contenido entra **solo aquí** y no por el transponedor: los exámenes viven en este botón.
...
No fundir dos cosas distintas: el **proceso** (Nivel 2) dice *cómo se trabaja* (fases, asistente, exámenes); el **perfil de transposición** (Nivel 4) dice *cómo se entrega* (audiencia, objetivo, foco, estructura). Una conferencia tiene un proceso de evento (N sesiones) *y* un perfil de transposición por sesión. Separarlos permite usar el perfil "seminario" dentro de un proceso de curso o de conferencia sin reescribir nada.

---

## 4. Dónde viven los exámenes: Opción A (cristalización), transversal

**Los siete exámenes se ejecutan al cristalizar el Estudio Madre (§2.4), no en la generación de artefactos.** Razón de producto: hay ~7 tipos de recurso y la Suite de Enseñanza es solo un consumidor. Si los exámenes vivieran en la skill, el bosquejo, el devocional y el ensayo saldrían sin auditar. Al vivir en el estudio, **todo lo que derive de él hereda la fidelidad** — el slide 7 del deck hecho código.
...
| **Exámenes de fidelidad** | no se ejecutan | nueva fn `validarEstudioMadre(elementos)` — corre en el botón "este es el contenido" |
...
Lo único que se inserta es: **transcript → [promoción] → elementos → [exámenes] → [serializar] → markdown.** De ahí, todo es el sistema que ya opera.
...
3. Los del flujo guiado nacen con elementos y exámenes.
...
**Procesos como manifiestos declarativos:** cada tipo declara `fases[]` (qué asistente, qué examen). El roadmap de la UI se renderiza desde ahí. Agregar un flujo ministerial = agregar un manifiesto, no recablear la app.

---

## 8. Los siete exámenes (corren en `validarEstudioMadre`, sobre los elementos)
...
Los distingue **quién carga la prueba.** A pregunta → el docente descubre (slots fijos). B el docente declara → el sistema exige respaldo (elementos componibles). Exámenes idénticos.
...
**B (mesa modular):** declaración (tesis, audiencia, objetivos, confesión) → módulos bajo demanda (estructura, argumento, contraargumento, exégesis dirigida sin versión rápida, ilustración, definición histórica) → mismos exámenes + auditoría de autoría con umbral (anti-Daniel para el experto).
...
- `validarEstudioMadre` con exámenes 1–4 + 7. `serializarEstudio → markdown`.
...
4. **Los exámenes viven en el recurso, no en la skill.** Por eso todo recurso hereda la fidelidad.
5. **Los siete exámenes son idénticos en todo arquetipo y todo destino.** Cambia el proceso (N2) y la entrega (N4); nunca el estándar (N3).
