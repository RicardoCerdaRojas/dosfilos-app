 ...
 * panels in the hero carousel. Strings come in as props.
 */
interface HeroChatMockProps {
    sessions: string[];
...
const SESSION_ACTIVE = [true, false, false, false, false];

export function HeroChatMock(props: HeroChatMockProps) {
    const {
        sessions: sessionTitles, sessionsTitle, userMessage, author, answerTitle,
        para1pre, para1emphasis, para2pre, para2ref, para2mid, para2post,
        bibliographyTitle, bibBook, continuePlaceholder,
    } = props;
    const sessions = sessionTitles.map((title, i) => ({ t: title, active: SESSION_ACTIVE[i] }));
        ...
        <div className="rounded-2xl border border-white/10 bg-[#0b1020] shadow-[0_30px_80px_-20px_rgba(0,0,0,0.6)] overflow-hidden">
            ...
            <div className="grid grid-cols-12 h-[500px]">
                <aside className="hidden md:block col-span-3 border-r border-white/5 bg-[#0a0f1c] p-4">
                    <div className="text-[11px] font-medium uppercase tracking-wider text-slate-500 mb-3 px-2">{sessionsTitle}</div>
                    <div className="space-y-1">
                        {sessions.map(({ t: title, active }) => (
                            <div
                                key={title}
                                className={`px-2.5 py-1.5 rounded-md text-[12px] truncate ${
                ...
                <main className="col-span-12 md:col-span-9 flex flex-col bg-[#0b1020]">
                    <div className="flex-1 overflow-hidden p-6 space-y-5">
                        ...
                        <div className="max-w-[88%]">
                            <div className="text-[11px] uppercase tracking-wider text-slate-500 mb-2">{author}</div>
                            <div className="rounded-xl bg-white/[0.03] border border-white/10 p-5 font-reading text-[13.5px] leading-relaxed text-slate-200">
                                ...
                                <p>
                                    {para2pre} <span className="text-emerald-400 underline decoration-dotted decoration-emerald-400/50 underline-offset-4">{para2ref}</span>{para2mid}{' '}
                                ...
                                <div className="mt-4 pt-4 border-t border-white/10">
                                    <div className="text-[10px] uppercase tracking-wider text-slate-500 mb-2">{bibliographyTitle}</div>
                                    <div className="text-[12px] font-sans flex items-baseline gap-2">
                                        ...
                                        <span className="text-slate-200">{bibBook}</span>
