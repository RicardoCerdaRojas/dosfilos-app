...
export function useAdminMetrics() {
    ...
    const fetchMetrics = useCallback(async () => {
        try {
            ...
            const dashboardMetrics: DashboardMetrics = {
                ...
                // DAU/MAU come from currentMonth (recalculated hourly)
                dau: aggregate.currentMonth?.dau || 0,
                mau: aggregate.currentMonth?.mau || 0,
                paidUsers: aggregate.currentMonth?.paidUsers || 0,
                newUsersToday: daily?.users?.new || 0,

                // Revenue
                mrr: aggregate.currentMonth?.mrr || 0,

                // Plan distribution — read from the daily doc (recalculated hourly)
                activeSubscriptions: {
                ...
                totalPreachingPlans: aggregate.allTime?.series || 0,
                // Total logins as DAU (unique users active today, as proxy)
                totalLogins: daily?.totalLogins || aggregate.currentMonth?.dau || 0,
    ...
    const refreshMetrics = useCallback(async () => {
        try {
            ...
            console.error('[useAdminMetrics] Error refreshing metrics:', err);
            // Still try to refetch even if the cloud function call failed
            await fetchMetrics();
        } finally {
            setRefreshing(false);
        }
