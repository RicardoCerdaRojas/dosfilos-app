...
/**
 * ASV (American Standard Version) Repository — adapter for English
 * Bible. The source JSON ships in `verses[]` shape (one row per
 * verse with numeric book id), so the constructor transforms it
 * into the canonical `BibleJSONData[]` shape that BaseJSONRepository
...
export class ASVRepository extends BaseJSONRepository {
    ...
    private transformData(asv: ASVData): BibleJSONData[] {
        ...
        for (const v of asv.verses) {
            if (!booksMap.has(v.book)) {
                booksMap.set(v.book, []);
            }
            const chapters = booksMap.get(v.book)!;
            const chapterIdx = v.chapter - 1;
            if (!chapters[chapterIdx]) {
                chapters[chapterIdx] = [];
            }
            chapters[chapterIdx][v.verse - 1] = v.text;
        }

        const result: BibleJSONData[] = [];
        booksMap.forEach((chapters, bookNum) => {
            result.push({ id: bookNum.toString(), chapters });
        });
        // Order by canonical book number so getBooks() returns them in
        // bible order without relying on Map iteration.
