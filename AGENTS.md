...
> Memoria canónica del proyecto. Léeme al inicio de cada sesión.
> La memoria de largo plazo buscable vive en `.byblos/memory.db` (vía el MCP `byblos-dev`):
> en vez de cargar todo aquí, **consulta** con `memory_search` lo relevante a tu tarea.
...
1. `session_start` con el objetivo de la sesión.
2. `memory_search` antes de actuar (no repitas trabajo, no contradigas decisiones).
3. Al descubrir un hecho/decisión/lección duradera: `memory_write` (entra como `proposed` hasta que el humano lo confirme).
4. Decisiones de arquitectura: `memory_write` kind=`decision`; para cambiar una previa usa `memory_supersede` (append-only).
5. `session_end` con un resumen de "dónde quedamos".

## Reglas (resumen; ver `.agent/rules/always-on.md`)

- (completar las directivas duras del proyecto)

## Tipos de memoria disponibles

`fact` · `glossary` · `directive` · `convention` · `rule` · `decision` · `episode` · `lesson` · `task` · `reference`

<!-- byblos:adopted -->

---

## Contexto migrado de CLAUDE.md
